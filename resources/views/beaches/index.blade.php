...
@section('title')
    {{ __('Beaches List View') }}
@endsection

@section('content')
    <!-- page title -->
    <x-page-title title="Beaches List View" pagetitle="Beaches" />
            ...
            <div class="card" id="beachesTable">
                <div class="card-body">
                    <div class="flex items-center">
                        ...
                        <div class="shrink-0">
                            <a href="{{ route('beaches.create') }}" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                <i data-lucide="plus" class="inline-block size-4"></i> <span class="align-middle">Add Beach</span>
                            </a>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            ...
                            <div class="xl:col-span-3">
                                <select name="status"
                                    ...
                                    <!-- Có thể thêm các trạng thái khác tùy vào nhu cầu -->
                                </select>
                            </div>
                ...
                <div class="card-body">
                        ...
                        <table class="w-full border-separate table-custom border-spacing-y-1 whitespace-nowrap">
                            <thead class="text-left">
                                <tr class="relative rounded-md bg-slate-100 dark:bg-zink-600">
                                    ...
                                    <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort" data-sort="country">Country</th>
                                    <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort" data-sort="location">Location</th>
                                    <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort" data-sort="created_at">Creation Date</th>
                            ...
                            <tbody class="list">
                                ...
                                <tr>
                                    <td class="px-3.5 py-2.5">
                                        @if ($beach->image_url)
                                            <img src="{{ asset($beach->image_url) }}" alt="{{ $beach->name }}" class="w-10 h-10 rounded-full img-thumbnail">
                                        @else
                                    ...
                                    <td class="px-3.5 py-2.5" data-sort="created_at">{{ $beach->created_at->format('Y-m-d H:i:s') }}</td>
                                    <td class="px-3.5 py-2.5 text-end">
                                        <div class="relative dropdown">
                                            ...
                                            <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600"
                                                ...
                                                <li>
                                                    <form action="{{ route('beaches.destroy', $beach->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    <div class="flex flex-col items-center mt-8 md:flex-row">
                        <div class="mb-4 grow md:mb-0">
                            <p class="text-slate-500 dark:text-zink-200">Showing <b>{{ $beaches->count() }}</b> of <b>{{ $beaches->total() }}</b> Results</p>
                        </div>
                        <ul class="flex flex-wrap items-center gap-2">
                            ...
                            @else
                                <li><a href="{{ $beaches->previousPageUrl() }}" class="inline-flex items-center justify-center bg-white text-slate-500 border border-slate-200 rounded px-4 py-2 hover:bg-custom-50">Previous</a></li>
                            @endif

                            @for ($i = 1; $i <= $beaches->lastPage(); $i++)
                                <li>
                                    <a href="{{ $beaches->url($i) }}" class="inline-flex items-center justify-center {{ ($beaches->currentPage() == $i) ? 'bg-custom-500 text-white' : 'bg-white text-slate-500 border border-slate-200' }} rounded px-4 py-2 hover:bg-custom-50">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endfor
                            ...
                            @if ($beaches->hasMorePages())
                                <li><a href="{{ $beaches->nextPageUrl() }}" class="inline-flex items-center justify-center bg-white text-slate-500 border border-slate-200 rounded px-4 py-2 hover:bg-custom-50">Next</a></li>
                            @else
