...
@section('content')
                        ...
                        @foreach ($feedbacks as $feedback)
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        class="relative dropdown inline-block text-left flex flex-col items-left justify-left">
    ...
    <ul class="flex flex-wrap items-center gap-2">
        ...
        @else
            <li><a href="{{ $feedbacks->previousPageUrl() }}" class="inline-flex items-center justify-center bg-white text-slate-500 border border-slate-200 rounded px-4 py-2 hover:bg-custom-50">Previous</a></li>
        @endif

        @for ($i = 1; $i <= $feedbacks->lastPage(); $i++)
            <li>
                <a href="{{ $feedbacks->url($i) }}" class="inline-flex items-center justify-center {{ $feedbacks->currentPage() == $i ? 'bg-custom-500 text-white' : 'bg-white text-slate-500 border border-slate-200' }} rounded px-4 py-2 hover:bg-custom-50">
                    {{ $i }}
                </a>
            </li>
        @endfor
        ...
        @if ($feedbacks->hasMorePages())
            <li><a href="{{ $feedbacks->nextPageUrl() }}" class="inline-flex items-center justify-center bg-white text-slate-500 border border-slate-200 rounded px-4 py-2 hover:bg-custom-50">Next</a></li>
        @else
    ...
    <div class="mt-5 w-full overflow-x-auto">
        <div class="card-body w-full">
            <h4 class="mb-3 font-semibold text-xl">All Blogs</h4>
            @if ($blogs->isEmpty())
                <p class="text-gray-500">No comments yet.</p>
            @else
                <table class="min-w-full w-full table-auto">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="px-6 py-3 text-left text-sm font-bold text-gray-600">Title</th>
                    ...
                    <tbody>
                        @foreach ($blogs as $blog)
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-6 py-4 text-gray-700">{{ $blog->title }}</td>

                                <td class="px-6 py-4 text-gray-500">{{ $blog->created_at->format('Y-m-d H:i:s') }}
                                </td>
        ...
        <!-- Phân trang cho Blogs -->
        <div class="flex flex-col items-center mt-8 md:flex-row">
            <div class="mb-4 grow md:mb-0">
                <p class="text-slate-500 dark:text-zink-200">Showing <b>{{ $blogs->count() }}</b> of
                    <b>{{ $blogs->total() }}</b> Results
                </p>
            </div>
            <ul class="flex flex-wrap items-center gap-2">
